...
- **图表面板全屏化**：查看数据时面板铺满整个 3D 视图区（分析数据时其他功能本来也不用），图表尺寸随窗口自适应重绘。
- **图表交互三件套**：滚轮以鼠标为中心缩放（±500 倍范围钳制）、左键拖拽平移、双击复位；关闭 uPlot 默认框选缩放让位平移。
## 2026-07-28 回退：机间避障功能移除
...
## 2026-07-28 地面站模型还原真实尺寸 + 平直着色

- 用户反馈放大后纹理失真、安装孔消失：QUE_SCALE 回到 1.0（真实 0.23m）。
- "肿块"伪影根因：平滑顶点法线在硬边 CAD 上插值出错。改为非索引平直着色（三角形 soup + 面法线），网格精度 0.3→0.15mm，安装孔/棱边清晰；代价是文件 2.1→8.4MB（本地加载无感）。解析器兼容空 idx。
## 2026-07-28 雀配色按真机照片校准

- 按真机照片（IMG20260226233454）：电池仓/保护圈淡紫(140,128,210)、底板/顶板/起落架/电机/螺柱黑、桨烟色。Gazebo 分组网格 gear_guard 拆为 gear+guard 分别配色；地面站同色系重烘焙。
## 2026-07-28 雀配色修正：紫罗兰色

- 用户指正：真机是紫罗兰色（非淡紫），且顶板也是紫色（初版误判为黑）。紫色统一改 (135,94,219)；Gazebo 分组 frame 拆出 top.stl 单独配色，底板保持黑。
## 2026-07-28 雀动力按电机台架数据标定
