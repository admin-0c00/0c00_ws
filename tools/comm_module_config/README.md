...
- 【导出配置】：把当前表单参数（含未写入的编辑）存成 JSON 文件，文件名含设备名/时间戳，可用于备份或批量配置。
- 【导入配置】：先选中目标设备再导入 JSON 文件，参数会填入表单，确认后点【写入配置】生效。**导入不覆盖目标设备的 MAC 地址**（防止把源模块的 MAC 错写到别的模块上）。
- 导入会校验文件标识（`_type`）和参数合法性，不合格的文件会被拒绝。
